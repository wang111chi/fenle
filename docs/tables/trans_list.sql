...
CREATE TABLE trans_list (
    ...
    `bank_id`             int              not null COMMENT '交易银行的数字编号 1001广发信用卡' ,
    ...
    `acnt_name`           varchar(16)      NOT NULL COMMENT '付款人姓名',
    `acnt_bankno`          varchar(32)      NOT NULL  COMMENT '交易卡号',
    `bankcard_type`       smallint(6)      NOT NULL COMMENT '0  信用卡',
    `user_type`           smallint(6)      NOT NULL COMMENT '0－个人',
    ...
    `standby5`            varchar(256)           default NULL,
     UNIQUE KEY `idx_sp_listid`(`spid`,`sp_tid`),
     KEY `idx_bank_acc`(`acnt_bankno`,`bankcard_type`),
