...
CREATE TABLE sp_bankroll_list (
        ...
        `bankroll_type` smallint comment '借贷类型：1-入 2-出 3-冻结 4-解冻',
        `state` int not null default 0 comment '状态，不同的type，对应不同的状态：如type=1时 1支付前，1支付中，2支付成功，4支付失败 5 退款 6冲动',
        `sp_account_no_hash` varchar(64) comment '卡号hash值',
        `user_name` varchar(64) comment '付款者姓名',
        `sp_balance` bigint NOT NULL DEFAULT 0 comment '当前余额（修改后的）',
