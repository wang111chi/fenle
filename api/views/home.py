...
def check_merchant(db, spid, cur_type):
    ...
    if merchant_ret is None:
        ...
        return True, {'merchant_pubkey': merchant_ret['rsa_pub_key']}
...
# 检查订单状态
def check_list(db, list_id, user_mobile):
    sel = select([
...
def cardpay_apply(db, safe_vars):
    ...
    if not ret_merchant[0]:
        return ApiJsonErrorResponse(ret_merchant[1])
    ...
    list_ret = db.execute(sel).first()
    merchant_pubkey = ret_merchant[1]['merchant_pubkey']
    ...
    if not ret_channel[0]:
        return ApiJsonErrorResponse(ret_channel[1])
    comput_data['bank_channel'] = ret_channel[1]['bank_channel']
    comput_data['bank_fee'] = safe_vars[
        'money'] * ret_channel[1]['bank_fee_percent'] / 10000
    ...
    if not ret_sp_bank[0]:
        return ApiJsonErrorResponse(ret_sp_bank[1])
    bank_spid = ret_sp_bank[1]['bank_spid']
    comput_data['fee']\
        = safe_vars['money'] * ret_sp_bank[1]['fee_percent'] / 10000
    ...
    if ret_channel[1]['is_need_mobile']:
        ...
        ret_data = util.encode_unicode(ret_data)
        cipher_data = util.rsa_sign_and_encrypt_params(
            ret_data, config.FENLE_PRIVATE_KEY, merchant_pubkey)
...
def cardpay_confirm(db, safe_vars):
    ...
    ret_data = util.encode_unicode(ret_data)
    cipher_data = util.rsa_sign_and_encrypt_params(
        ret_data, config.FENLE_PRIVATE_KEY, merchant_ret['rsa_pub_key'])
